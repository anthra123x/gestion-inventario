...
export default function FinancesPage() {
  ...
  const savingsProgress =
    summary?.savingGoals?.length > 0
      ? summary.savingGoals.reduce((sum: number, g: any) => sum + (g.currentAmount / g.targetAmount) * 100, 0) /
        summary.savingGoals.length
      : 0
          ...
          <div className="space-y-6">
            ...
            {dailySummary?.expenseByCategory?.length > 0 && (
              <Card className="card-shadow border-border/60">
                <CardHeader>
                  <SectionHeader icon={PieChart} title="Gastos por Categoría" description="Hoy" />
                </CardHeader>
                <CardContent>
                    ...
                    <ResponsiveContainer width="100%" height={140}>
                      <RePieChart>
                        <Pie
                          ...
                          paddingAngle={2}
                          dataKey="amount"
                          data={dailySummary.expenseByCategory}
                        >
                  ...
                  <div className="space-y-2">
                    {dailySummary.transactions.map((tx: any) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div
                            className={`w-2 h-2 rounded-full ${tx.type === 'INCOME' ? 'bg-green-500' : 'bg-red-500'}`}
                          />
                          <div>
                        ...
                        <span
                          className={`text-sm font-bold tabular-nums ${tx.type === 'INCOME' ? 'text-green-600' : 'text-red-600'}`}
                        >
                          {tx.type === 'INCOME' ? '+' : '-'}
              ...
              <Card className="card-shadow border-border/60">
                <CardContent>
                  <EmptyState
                    ...
                    description="No hay transacciones registradas para el día de hoy."
                  />
                </CardContent>
              </Card>
            )}
          ...
          <div className="space-y-6">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-sm font-semibold">
                    Semana del{' '}
                    {new Date(periodSummary.period.startDate).toLocaleDateString('es-CO', {
                      day: 'numeric',
                      month: 'short',
                    })}{' '}
                    al{' '}
                    {new Date(periodSummary.period.endDate).toLocaleDateString('es-CO', {
                      day: 'numeric',
                      month: 'short',
                      year: 'numeric',
                    })}
              ...
              <Card className="card-shadow border-border/60">
                ...
                <CardContent>
                  <ResponsiveContainer width="100%" height={220}>
                    <RechartsBarChart
                      data={periodSummary.dailyBreakdown}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-lg font-bold text-emerald-600 tabular-nums">
                        {formatCurrency(periodSummary.period.savingsAllocated)}
                      </p>
                    </div>
                  </div>
                </CardContent>
            ...
            <div className="flex justify-end">
              <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                <DialogTrigger render={<Button variant="outline" size="sm" />}>Cerrar Semana</DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                  ...
                  <form onSubmit={handleCloseWeek} className="space-y-4 pt-2">
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground mt-1">
                        Si no se especifica, se ahorrará el balance completo.
                      </p>
                    </div>
          ...
          <div className="space-y-6">
            <StatCardGrid>
              ...
              <StatCard
                ...
                change={
                  summary?.savingGoals?.length > 0 ? `${Math.round(savingsProgress)}% de progreso promedio` : undefined
                }
                icon={PiggyBank}
                color="purple"
              />
            ...
            <div className="grid gap-4 grid-cols-1 lg:grid-cols-2 xl:grid-cols-4">
              <Card className="card-shadow border-border/60 xl:col-span-2">
                <CardHeader>
                  <SectionHeader icon={BarChart3} title="Balance Mensual" description="Últimos 12 meses" />
                </CardHeader>
                <CardContent>
              ...
              <Card className="card-shadow border-border/60">
                <CardHeader>
                  <SectionHeader icon={PieChart} title="Gastos por Categoría" description="Este mes" />
                </CardHeader>
                <CardContent>
              ...
              <Card className="card-shadow border-border/60">
                <CardHeader>
                  <SectionHeader icon={Target} title="Metas de Ahorro" description="Progreso" />
                </CardHeader>
                <CardContent>
                  {!summary?.savingGoals || summary.savingGoals.length === 0 ? (
                    <EmptyState
                      ...
                      action={{ label: 'Crear Meta', href: '/finances/goals' }}
                    />
                  ) : (
            ...
            <Card className="card-shadow border-border/60">
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Link href="/finances/transactions" className="text-xs text-primary hover:underline">
                    Ver todas
                  </Link>
                </div>
              </CardHeader>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {tx.category?.name || 'Sin categoría'} · {new Date(tx.date).toLocaleDateString('es-CO')}
                            </p>
                          </div>
                        </div>
                        <span
